...
/**
 * Code for testing "lazy binding" - only binding values into a JavaScript context if they'll be
 * referenced in an evaluation. The reason for this is that binding in a Graal polyglot Context is
 * very slow compared to Rhino.
 * 
 * 
 */
public class LazyBinding {
   ...
   * snippets using both Rhino and Graal (any others...?)
   */
  ...
  /**
   * For {@link Context#eval(Source)} we need to create a Source object, this creates a factory that
   * creates a Source instanced for every snippet.
   * 
   * @return
   */
  ...
  /**
   * For {@link Context#eval(Source)} we need to create a Source object, this creates a factory that
   * will return the same Source instance every time a given snippet is seen.
   * 
   * @return
   */
  ...
  /**
   * return a factory to create an Evaluator that uses graaljs. Since creating a Context is slow,
   * Evaluators created by this factory all use the same underlying context (OK as we're just going
   * to be using a single thread).
   * 
  ...
  private static Supplier<Evaluator> rhinoEvaluatorFactory() {
    ...
    engine.getBindings(ScriptContext.ENGINE_SCOPE).put("org.mozilla.javascript.optimization_level",
        "-1");
    return () -> {
  ...
  /**
   * Naive unique token extraction (assumes whitespace around them and doesn't take account of
   * parenthesis or operators). e.g.
   * 
   * <pre>
  ...
  /**
   * Iterates over some snippets of code, evaluating each one <code>iterations</code> times. Each
   * evaluation creates a new <code>Evaluator</code> using the given factory and binds variables to
   * it using the given <code>binder</code>.
   * 
   ...
   */
  private static Duration test(Supplier<Evaluator> evaluatorFactory,
      BiConsumer<Evaluator, String> binder, int iterations) {
  ...
  /**
   * Creates an Evaluator; binds values into it; evaluates the code snippet; closes the evaluator
   * 
   * @param evaluatorFactory
   * @param binder
   * @param snippet
